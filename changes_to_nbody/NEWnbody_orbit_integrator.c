...
void nbReverseOrbit_LMC(mwvector* finalPos,
                    ...
                    real dt)
{
    mwvector acc, v, x, LMCacc, LMCv, LMCx;
    ...
    // Get the initial acceleration
    mw_acc = pointAccel(449865.888, mw_x, LMCx);
    LMC_acc = nbExtAcceleration(pot, LMCx);
    acc = mw_addv(nbExtAcceleration(pot, x), pointAccel(449865.888, x, LMCx));

    // Shift the body
    mw_incaddv(LMC_acc, mw_incnegv(mw_acc));
    mw_incaddv(acc, mw_incnegv(mw_acc));
