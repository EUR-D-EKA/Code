...
mwvector dynamicalFriction(mwvector pos, mwvector vel, const Potential* pot){
    mwvector result; //Vector with change in velocity due to DF

    real LMC_mass = mw_pow(10, 11) * 1.8*mw_pow(10, 30); //Test mass (for now)
   	real temp_N = 2000; //Test n bodies (for now) (related to density)

    real const G_CONST = 6.67*mw_pow(10, -11); //figure out better place to get this from
    real k = 1.428; //?
    real r = mw_pow(mw_dotv(pos,pos), 0.5); //?
    int a = 12; //?
    int v0 = 73; //should this be the magnitude of vel

    //object velocity , center of gravity is initially at rest
    ...
    real density = LMC_mass * temp_N;
    //force from DF
    real F = (-4*pi*mw_pow(G_CONST, 2)*mw_pow(LMC_mass, 2)* mw_log(lambda)*density / mw_pow(objectVel, 3)) * (erf(X) - 2*X/mw_pow(pi, 0.5)*exp(mw_pow(-1*X, 2)));
